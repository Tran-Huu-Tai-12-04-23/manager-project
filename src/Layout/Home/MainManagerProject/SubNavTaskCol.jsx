...
function SubNavTaskCol({
    ...
    const [openModalConfirmRemoveCol, setOpenModalConfirmRemoveCol] = useState(false);
    const dispatch = useDispatch();
    const [waitRemoveCol, setWaitRemoveCol] = useState(false);
    const handleRemoveCol = async () => {
        if (dataCol && projectDetail) {
            const idCol = dataCol._id;
            setWaitRemoveCol(true);
            const result = await Service.remove('/col/remove-col', `/?colId=${idCol}&projectId=${projectDetail._id}`);
            ...
            setWaitRemoveCol(false);
        }
    };
    return (
        <>
            <Modal open={openModalConfirmRemoveCol} setOpen={setOpenModalConfirmRemoveCol}>
                <ModalConfirmRemove
