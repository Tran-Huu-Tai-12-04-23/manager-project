...
function CardProjectTrash({ data }) {
    ...
    const handleRemoveProject = async () => {
        const result = await Service.remove('/project/remove-project', `/?projectId=${data._id}`);
        ...
        if (result.status === true) {
            ...
            dispatch(projectTrashAction.remove({ projectId: data._id }));
            dispatch(numberTrashAction.decrease({ number: 1 }));
            setOpenModalConfirmRemove(false);
        } else {
    ...
    const restoreProject = async () => {
        const result = await Service.update('/project/restore-project', {
            projectId: data._id,
        });
    ...
    return (
        ...
        >
            ...
            <div className="flex justify-start items-center mt-5">
                ...
                <Tooltip title="Xóa">
                    <button
                        onClick={(e) => setOpenModalConfirmRemove(true)}
                        className="text-xs mr-2 text-red-500 rounded-md bg-ux p-2 hover:brightness-125"
                    >
                        <BsTrash></BsTrash>
                    </button>
                </Tooltip>
