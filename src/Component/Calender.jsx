...
const Calendar = () => {
  ...
  const onDragEnd = (result) => {
    ...
    console.log({
      res, dispatch, taskAction, tasks ,dateInWeek,projectAction
    })
    ...
    if (idSource !== idDes && idDes && tasks) {
      ...
      if( date) {
        dispatch(taskAction.updateDate({
          newDate: date.date.toISOString(), id: idTask
        }))
        setDataChange({
          taskId: idTask,newDate: date.date.toISOString(),
        })
      }
        ...
        >
          ...
          {Array.isArray(dateInWeek) && 
            dateInWeek.map(( d , index) => {
              ...
              return <DayOfWeek key={index} day={day} weekday={weekday} checkSameDay={checkSameDay} date={d.date} id={d.id} year={year} tasks={tasks}> 
              </DayOfWeek>
            })
          }
          </div>
