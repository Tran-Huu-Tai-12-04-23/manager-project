...
function FormAddNewMember({ action }) {
    ...
    useEffect(() => {
        const initMember = async () => {
            const result = await Service.getDataFromApi(`/user/get-users-not-member/?projectId=${projectDetail._id}`);
            setListUser(
    ...
    const handleAddMember = async () => {
        ...
        if (projectDetail && userSelect) {
            ...
            if (result.data.status === true) {
                ...
                dispatch(projectDetailAction.initProjectDetail(JSON.parse(newProject.data)));
            } else {
    ...
    return (
        ...
        >
            <div className="w-full">
                ...
                <h5 className="text-md font-bold w-fit">Mời người khác vào cùng làm việc</h5>
                                ...
                                return (
                                    <li
                                        ...
                                        onClick={(e) => {
                                            ...
                                            setSuggestResult(false);
                                            setUserSelect(user);
                                            setEmailOrName(user.email ? user.email : user?.displayName);
                                        }}
                ...
                <div className="justify-start items-center flex mt-3">
                    {!link && (
                        <>
                            ...
                            <h5 className="text-xs font-bold w-fit">Mời thêm thành viên vào thông qua liên kết này</h5>
                            {waitCreateLink && (
                                <button className="reset w-24 text-xs ml-auto rounded-md hover:brightness-125 bg-light-third dark:bg-dark-third p-2">
                                    <CircularProgress
                                        sx={{
                    ...
                    {link && (
                        <>
                            ...
                            <h5 className="underline text-xs font-bold w-fit text-primary">{link}</h5>
                            <CopyToClipboard text={link} onCopy={handleCopy}>
                                <button className="reset text-xs w-24 flex justify-center items-center ml-auto rounded-md hover:brightness-125 bg-light-third dark:bg-dark-third p-2">
