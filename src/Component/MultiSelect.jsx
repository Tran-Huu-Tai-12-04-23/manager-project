...
function MultiSelect({ className, width = 'full', memberSelected, setMemberSelected }) {
    ...
    useEffect(() => {
        const initMember = async () => {
            const result = await Service.getDataFromApi(`/user/get-users/?userId=${dataLogin.id}`);
            console.log(result);
            setMembers(
    ...
    useEffect(() => {
        const filteredMembers = members.filter((member) => {
            const checkMemberAlreadyExists = memberSelected.find((mem) => mem._id === member._id);
        ...
        >
            {memberSelected &&
                ...
                memberSelected.map((member, index) => {
                    return (
                        <div
                            key={index}
                            className=" max-w-[10rem] w-[10rem] overflow-hidden rounded-xl mt-2 scale-95 flex justify-start items-center bg-design p-1 pl-2 pr-2"
                        >
                            <h5 className="text-xs mr-2 w-full overflow-hidden text-ellipsis">
            ...
            {open && (
                ...
                >
                    ...
                    {members.length === 0 && <div className="h-32 p-4">Không có thành viên hợp lệ!</div>}
                </div>
            )}
        </div>
    );
